    ...
    const exportData = {
      metadata: {
        ...
        formulas_detailed: {
          ...
          demand: {
            formula: "Попит = населення × 0.15 × 3; Пропозиція = пункти × 50 × 180 × 2",
            scoring_scale: {
              "ratio < 0.6": "25 балів (дефіцит >40%)",
              "ratio 0.6-0.8": "20 балів (дефіцит 20-40%)",
              "ratio 0.8-1.0": "15 балів (баланс)",
              "ratio 1.0-1.5": "10 балів (надлишок 0-50%)",
              "ratio > 1.5": "0 балів (перенасичення)"
            },
            weight: "25% (найвищий пріоритет за AHP)",
          ...
          pfz: {
            formula: "НПП×2.0 + Заповідники×1.5 + РЛП×1.0 + Заказники×0.1 + Пам'ятки×0.05 + Бонус_площі",
            max_score: 20,
            weight: "20% (згідно AHP)",
            ...
            weights_rationale: {
              ...
              zakazniks: "×0.1 - локальне значення, обмежений туристичний потенціал",
              monuments: "×0.05 - поодинокі об'єкти",
              area_bonus: "+1-2 бали якщо ПЗФ займає >5% території",
          ...
          transport: {
            formula: "Базова(5) + Залізниці(0-5) + Міжнародні_траси(0-3) + Аеропорти(0-1) + Бонус_щільності(0-1)",
            max_score: 15,
            weight: "15% (згідно AHP)",
            components: {
              base: "5 балів (базова доступність)",
              railways: "0-5 балів (≥50 станцій=5, ≥30=3, ≥10=1)",
              international_roads: "0-3 бали (кількість міжнародних трас)",
              airports: "0-1 бал (наявність аеропорту)",
              density_bonus: "0-1 бал (щільність доріг >250 км/1000км²)"
            },
          ...
          fire_prevention: {
            formula: "if пожежі_людські >= 15: 5 балів; >= 10: 3 бали; >= 5: 1 бал; < 5: 0 балів",
            max_score: 5,
            weight: "+5% (бонус за AHP)",
            scoring_scale: {
              "≥15 пожеж": "5 балів (критична потреба в облаштуванні)",
              "10-14 пожеж": "3 бали (висока потреба)",
              "5-9 пожеж": "1 бал (помірна потреба)",
              "<5 пожеж": "0 балів (низька потреба)"
            },
          ...
          saturation: {
            formula: "Прогресивна шкала штрафів залежно від щільності існуючих об'єктів",
            max_penalty: -15,
            weight: "-15% (штраф за AHP)",
            scale: {
              very_high: ">6.0 об'єктів/1000км² = -15 балів (критичне перенасичення)",
              high: "4.0-6.0 = -10 балів (високе)",
              moderate_high: "3.0-4.0 = -7 балів (помірно-високе)",
              moderate: "2.0-3.0 = -5 балів (помірне)",
              low_moderate: "1.0-2.0 = -3 бали (низько-помірне)",
              low: "0.5-1.0 = -2 бали (низьке)",
              very_low: "<0.5 = 0 балів (мінімальна конкуренція)"
            },
            justification: "Висока концентрація → менше місця для нових → нижчий потенціал (Kentucky Market Analysis)"
          }
        },
        zone_identification_algorithm: {
          title: "Алгоритм визначення оптимальних локацій на карті",
          description: "Система працює на ДВОХ рівнях: регіональному (оцінка областей) та локальному (конкретні території)",
          levels: {
            regional: {
              name: "Регіональний рівень (Області України)",
              purpose: "Оцінка рекреаційного потенціалу 24 областей",
              formula: "7 факторів (попит, ПЗФ, природа, транспорт, інфраструктура, пожежі, насиченість)",
              result: "Бал 0-100 для кожної області",
              example: "Київська область = 78.5 балів"
            },
            local: {
              name: "Локальний рівень (Конкретні території на карті)",
              purpose: "Пошук оптимальних локацій для рекреаційних об'єктів",
              types: "3 типи зон: near_pfz, roadside, fire_prevention",
              result: "Точки на карті з пріоритетами 0-100",
              example: "Зона біля НПП 'Синевир' = 78 балів пріоритету"
            }
          },
          zone_types: {
            near_pfz: {
              name: "Near PFZ (поблизу ПЗФ)",
              description: "Локації в радіусі 5-15 км від об'єктів ПЗФ",
              base_priority: 60,
              max_priority: 100,
              logic: "Туристи хочуть бути близько до національних парків, заповідників",
              examples: ["Біля НПП Карпатський", "Поруч з РЛП Гуцульщина"]
            },
            roadside: {
              name: "Roadside (вздовж доріг)",
              description: "Локації вздовж міжнародних трас (М01-М29, Е40, Е50...)",
              base_priority: 50,
              max_priority: 80,
              logic: "Транзитні туристи потребують зупинок, відпочинку, харчування",
              examples: ["Траса Київ-Чоп (М06)", "Траса Львів-Краковець (М10)"]
            },
            fire_prevention: {
              name: "Fire Prevention (профілактика пожеж)",
              description: "Локації в пожежонебезпечних лісових масивах",
              base_priority: 40,
              max_priority: 70,
              logic: "Облаштовані місця з вогнищами → зменшення хаотичних пожеж на 40%",
              examples: ["Житомирська область (95 пожеж/рік)", "Київська область (33 людські пожежі)"]
            }
          },
          priority_formula: {
            formula: "Priority = Base_Type + PFZ_Bonus + Infrastructure + Fire_Prevention - Competition",
            components: {
              base_type_priority: {
                near_pfz: 60,
                roadside: 50,
              ...
              pfz_proximity_bonus: {
                description: "Бонус за близькість до ПЗФ",
                scale: {
                  "< 5 км від НПП": "+20 балів",
                  "< 10 км від РЛП": "+10 балів",
                  "< 15 км від заказника": "+5 балів"
                }
              },
              infrastructure_quality: {
                description: "Якість місцевої інфраструктури",
                range: "0-10 балів",
                factors: ["Близькість доріг", "Наявність заправок", "Доступ до лікарень"]
              },
              fire_prevention_bonus: {
                description: "Бонус за потребу в профілактиці пожеж",
                range: "0-10 балів",
                logic: "Більше пожеж в районі = вища потреба в облаштуванні"
              },
              competition_penalty: {
                description: "Штраф за існуючі об'єкти поблизу",
                range: "0-30 балів",
                logic: "-10 балів за кожен об'єкт в радіусі 10 км"
              }
            },
            example_calculation: {
              scenario: "Зона A (near_pfz біля НПП 'Синевир')",
              calculation: [
                "Base Priority: 60 (near_pfz)",
                "+ PFZ Bonus: 20 (< 5 км від НПП)",
                "+ Infrastructure: 8 (добра доступність)",
                "+ Fire Prevention: 0 (немає проблеми пожеж)",
                "- Competition: -10 (1 конкурент в радіусі)",
                "= TOTAL: 78 балів (ВИСОКИЙ пріоритет)"
              ]
            }
          },
          visualization: {
            map_markers: {
              high_priority: "Зелені маркери (70-100 балів) - найкращі локації",
              medium_priority: "Жовті маркери (50-69 балів) - хороші локації",
              low_priority: "Червоні маркери (0-49 балів) - менш привабливі"
            },
            filtering: "Користувач може фільтрувати зони за типом та пріоритетом"
          }
        },
        weights_summary: {
          demand: "25% (найвищий)",
