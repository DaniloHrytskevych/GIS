...
Пам'ятки: ${d?.pfz?.monuments_of_nature || 0} × 0.05 = ${((d?.pfz?.monuments_of_nature || 0) * 0.05).toFixed(2)}</code>
  ...
  <div class="section">
    ...
    <div class="step-box">
      <h4>Формула розрахунку:</h4>
      <code>Бали = Лікарні(0-3) + Заправки(0-2) + Мобільний_зв'язок(0-2) + Інтернет(0-1) + Готелі(0-1) + Електрика(0-1)</code>
      <p style="margin-top: 8px;"><strong>Компоненти (максимум 10):</strong></p>
      <ul style="font-size: 11pt;">
        <li>Лікарні: 0-3 бали (за щільністю на 100k населення)</li>
        <li>Заправки: 0-2 бали (за щільністю на 1000 км²)</li>
        <li>Мобільний зв'язок: 0-2 бали (покриття %)</li>
        <li>Інтернет: 0-1 бал (доступність)</li>
        <li>Готелі/житло: 0-1 бал (наявність)</li>
        <li>Електропостачання: 0-1 бал (надійність)</li>
      </ul>
